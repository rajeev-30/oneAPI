  ...
  const maxCost = useMemo(
    () =>
      !usage?.modelBreakdown?.length
        ? 1
        : Math.max(...usage.modelBreakdown.map((m) => m.cost)),
    [usage],
  );
    ...
    <div className="p-4 max-w-4xl mx-auto space-y-6 animate-fade-in">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-text-muted">
            Your usage across models on oneAPI
          </p>
        </div>
        <div className="flex items-center gap-1">
          ...
          <button
            ...
            className="p-1.5 rounded-md text-text-muted hover:text-text-primary hover:bg-white/[0.06] transition-colors disabled:opacity-30 cursor-pointer disabled:cursor-not-allowed">
      ...
      {isLoading ? (
        ...
        <Card className="text-center py-16">
          ...
          <p className="text-sm text-text-muted">No data in this window</p>
        </Card>
      ) : (
        <>
            ...
            <Card>
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-lg bg-accent-amber/10 flex items-center justify-center">
                  <Coins size={18} className="text-accent-amber" />
                </div>
                <div>
            ...
            <Card>
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-lg bg-accent-violet/10 flex items-center justify-center">
                  <TrendingUp size={18} className="text-accent-violet" />
                </div>
                <div>
                    ...
                    <div key={i} className="space-y-2">
                      <div className="flex items-center justify-between text-sm">
                        <div className="flex items-center gap-2">
                          ...
                          <Badge>{item.model?.provider?.name || "—"}</Badge>
                        </div>
