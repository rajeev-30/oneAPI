...
export default function BillingPage() {
  ...
  const { data: wallet, isLoading: wLoad } = useQuery({
    queryKey: ["wallet"],
    queryFn: getWallet,
    retry: false,
  });
  const { data: sub, isLoading: sLoad } = useQuery({
  ...
  const topup = useMutation({
    ...
    onError: (e) =>
      toast.error(e instanceof Error ? e.message : "Credits top-up failed"),
  });

  return (
    <div className="p-6 max-w-3xl mx-auto space-y-6 animate-fade-in">
      <div>
        <h1 className="text-3xl font-semibold text-text-primary">Credits</h1>
      ...
      {wLoad ? (
        ...
        <Card className="bg-gradient-to-br from-brand-900/50 to-brand-800/30 border-brand-700/30">
          ...
          <div className="mt-4">
            <Button onClick={() => setShowTopup(true)} size="md">
        ...
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              <Shield size={16} className="text-brand-400" /> Subscription
            </CardTitle>
          ...
          <CardContent>
            <div className="grid grid-cols-2 gap-4 text-sm">
              ...
              <div>
                <p className="text-text-muted text-xs">End</p>
                <p className="text-text-primary text-base">
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-4 gap-2">
            {[50, 100, 500, 1000].map((a) => (
              <button
                ...
                className={`py-2 rounded-lg border text-sm font-medium transition-all cursor-pointer ${amount === String(a) ? "border-brand-500 bg-brand-500/10 text-brand-400" : "border-border-primary text-text-secondary hover:border-border-active"}`}>
                ₹{a}
              </button>
            ))}
          </div>
          ...
          <div className="flex gap-2 justify-end">
            ...
            <Button
              ...
              disabled={!amount || Number(amount) < MINIMUM_TOPUP_AMOUNT}
