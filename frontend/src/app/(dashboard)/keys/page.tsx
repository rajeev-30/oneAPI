      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-text-muted">
            Manage your API keys for accessing oneAPI
          </p>
        </div>
            ...
            <Card key={key._id} className="group">
                  ...
                  {editingId === key._id ? (
                    <div className="flex items-center gap-2">
                      <Input
                        ...
                        onKeyDown={(e) => {
                          if (e.key === "Enter")
                            updateMut.mutate({ id: key._id, name: editName });
                    ...
                    <>
                      ...
                      <button
                        ...
                        className="opacity-0 group-hover:opacity-100 p-1 rounded hover:bg-white/[0.06] text-text-muted transition-all cursor-pointer">
                        <Pencil size={12} />
                      </button>
                    </>
                  )}
              ...
              <div className="flex items-center gap-2 mb-3">
                ...
                <button
                  ...
                  className="p-1.5 rounded-md text-text-muted hover:text-text-primary hover:bg-white/[0.06] transition-colors cursor-pointer">
                  {visible.has(key._id) ? (
                    <EyeOff size={14} />
                  ) : (
                    <Eye size={14} />
                ...
                <button
                  onClick={() => deleteMut.mutate(key._id)}
                  className="p-1.5 rounded-md text-text-muted hover:text-accent-rose hover:bg-accent-rose/10 transition-colors cursor-pointer">
                  <Trash2 size={14} />
                </button>
              </div>
              <div className="flex gap-6 text-xs text-text-muted">
                ...
                <span>
                  Tokens:{" "}
                  <span className="text-text-secondary">
                    {formatTokens(key.totalTokensUsed)}
                  </span>
                </span>
                <span>
                  Spent:{" "}
                  <span className="text-text-secondary">
                    {formatCurrency(key.totalSpent)}
                  </span>
                </span>
                <span>
                  Last used:{" "}
      ...
      <Dialog
        ...
        <div className="space-y-4">
          ...
          <div className="flex gap-2 justify-end">
            ...
            <Button
              onClick={() => createMut.mutate(newKeyName.trim())}
