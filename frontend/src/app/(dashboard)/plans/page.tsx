        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {plans.map((plan) => {
            ...
            return (
              <Card
                key={plan._id}
                className={`flex flex-col ${isCurrent ? "border-brand-500/50 ring-1 ring-brand-500/20" : ""}`}>
                ...
                <div className="mt-4 space-y-2 flex-1">
                  {plan.features?.map((f, i) => (
                    <div
                      ...
                      <Check
                        size={12}
                        className="text-accent-emerald shrink-0"
                      />{" "}
                      {f}
                    </div>
                  ))}
                  <div className="pt-2 space-y-1 text-[11px] text-text-muted">
                    <p>
                      ↳ {plan.limits.requestsPerDay.toLocaleString()} req/day
                    </p>
                    <p>
                      ↳ {plan.limits.tokensPerDay.toLocaleString()} tokens/day
                    </p>
