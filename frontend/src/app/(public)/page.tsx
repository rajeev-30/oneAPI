...
import { Zap, Shield, BarChart3, Globe, ArrowRight, MoveRight } from "lucide-react";
...
export default function LandingPage() {
  ...
  return (
    <div className="min-h-screen">
      ...
      <section className="relative pt-24 pb-20 px-4 text-center overflow-hidden">
        ...
        <div className="relative z-10 max-w-4xl mx-auto">
          ...
          <p className="mt-5 text-lg text-text-secondary max-w-xl mx-auto">
            Better <Link href="/plans" className="gradient-text">pricing</Link>, better uptime. One <span className="gradient-text">API</span> for every model.
          </p>
          <div className="flex items-center justify-center gap-3 mt-6">
            <Link
              href="/keys"
              className="inline-flex min-w-[220px] justify-center items-center gap-2 px-6 py-2.5 rounded-md bg-brand-500 text-white font-bold text-sm hover:bg-brand-600 transition-colors shadow-lg shadow-brand-500/25"
            >
            ...
            <Link
              href="/models"
              className="inline-flex min-w-[220px] justify-center items-center gap-2 px-6 py-2.5 rounded-md border border-border-primary text-text-secondary font-bold text-sm hover:text-text-primary hover:bg-white/[0.06] transition-all duration-200"
            >
      ...
      {/* Stats */}
      <section className="py-2 border-y border-border-secondary">
        <div className="max-w-4xl mx-auto px-35 grid grid-cols-2 md:grid-cols-4 gap-6 text-center">
          {[
            ...
            <div key={stat.label}>
              <p className="text-lg md:text-4xl font-black gradient-text">{stat.value}</p>
        ...
        <div className="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5">
          {[
            ...
            >
              ...
              <p className="text-sm text-text-secondary leading-relaxed mb-18">{f.desc}</p>
              <Link href={f.href} className="flex gap-1 items-center text-sm text-brand-400 hover:text-brand-300 font-medium transition-colors">
      ...
      <footer className="border-t border-border-secondary py-8 px-4 text-center">
        <p className="text-sm text-text-secondary">© 2026 oneAPI. Made with 💙 for developers.</p>
      </footer>
    </div>
  );
}
