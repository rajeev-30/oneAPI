...
export default function ChatPage() {
  ...
  const { messages, selectedModel, isStreaming, activeConversationId } =
  ...
  const { data: convTitles } = useQuery({
    queryKey: ["conversations"],
    queryFn: () => getConversationTitles(1, "all"),
    staleTime: 30 * 1000,
  });
  ...
  useEffect(() => {
    if (keysData?.length) setApiKey(keysData[0].key);
  }, [keysData]);
  useEffect(() => {
    if (conv) {
    ...
    return (
      <div className="flex-1 h-full flex flex-col items-center justify-center text-center">
        ...
        <p className="text-sm text-text-muted max-w-sm mb-4">
          Generate an API key to use the chat playground.
        </p>
        <Link
          href="/keys"
    ...
    <div className="flex h-full min-h-0 overflow-hidden">
      ...
      <div
        className={cn(
          ...
          sidebarCollapsed
            ? isMobile
              ? "-translate-x-full w-70 opacity-0 border-r-0"
        ...
        <div className="p-2 border-b border-border-secondary">
          <button
            ...
            className="flex items-center gap-2 w-full px-2 py-2 rounded-md text-xs font-medium text-text-primary bg-brand-500/10 hover:bg-brand-500/20 border border-brand-500/20 transition-colors cursor-pointer">
        ...
        <div className="flex-1 overflow-y-auto p-2.5 space-y-1.5">
          {filteredTitles.length > 0 ? (
            ...
            <div className="text-sm text-text-secondary h-full flex justify-center items-center my-auto">
