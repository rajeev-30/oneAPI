    ...
    <div className="p-6 space-y-5 animate-fade-in">
      <div>
        ...
        <p className="text-sm text-text-muted">
          {pagination ? `${pagination.total_items} keys across all users` : "All API keys on the platform"}
        </p>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="text-center py-16 text-text-muted text-sm">No API keys generated yet.</div>
      ) : (
        <div className="overflow-x-auto rounded-xl border border-border-primary bg-bg-card">
          <table className="w-full text-sm">
            ...
            <tbody>
              {keys.map((k) => {
                const owner = typeof k.user === "object" ? k.user as User : null;
                return (
                  <tr key={k._id} className="border-b border-border-primary/50 hover:bg-white/[0.02]">
                    <td className="py-3 px-4">
                      <div className="flex items-center gap-2">
                        <div className="w-6 h-6 rounded bg-accent-amber/10 flex items-center justify-center"><Key size={11} className="text-accent-amber" /></div>
                        <span className="font-medium text-xs">{k.name}</span>
                      </div>
                    </td>
                    <td className="py-3 px-4 text-text-secondary text-xs">{owner?.name || "—"}<br /><span className="text-text-muted text-[10px]">{owner?.email || ""}</span></td>
                    <td className="py-3 px-4 text-text-muted font-mono text-[10px]">{k.key.slice(0, 16)}...{k.key.slice(-4)}</td>
                    <td className="py-3 px-4 text-right text-text-secondary text-xs font-mono">{k.totalRequests.toLocaleString()}</td>
                    <td className="py-3 px-4 text-right text-text-secondary text-xs font-mono">{k.totalTokensUsed.toLocaleString()}</td>
                    <td className="py-3 px-4 text-right text-text-secondary text-xs font-mono">₹{k.totalSpent.toFixed(2)}</td>
                    <td className="py-3 px-4 text-text-muted text-[11px]">
                      {k.lastUsedAt ? new Date(k.lastUsedAt).toLocaleDateString("en-IN", { day: "2-digit", month: "short" }) : "Never"}
                    </td>
                  </tr>
                );
              })}
