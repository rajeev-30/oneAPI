...
function Modal({
  ...
  const [f, setF] = useState<BForm>({
    name: b?.name || "",
    input: b?.inputCostPer1KTokens?.toString() || "",
    output: b?.outputCostPer1KTokens?.toString() || "",
    ...
    <div
      ...
      <div
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input
                ...
                step="0.001"
                value={f.input}
                onChange={(e) => setF((p) => ({ ...p, input: e.target.value }))}
                className="w-full px-3 py-2 rounded-lg border border-border-primary bg-bg-secondary text-sm focus:outline-none focus:border-border-active"
              />
            </div>
            <div>
              ...
              <input
                ...
                value={f.output}
                onChange={(e) =>
                  setF((p) => ({ ...p, output: e.target.value }))
                }
                className="w-full px-3 py-2 rounded-lg border border-border-primary bg-bg-secondary text-sm focus:outline-none focus:border-border-active"
              />
            </div>
        ...
        <div className="flex justify-end gap-2 mt-5">
          ...
          <button
            onClick={() => f.name && f.input && f.output && onSave(f)}
            disabled={!f.name || !f.input || !f.output}
...
export default function BillingPage() {
  ...
  const [modal, setModal] = useState<{ open: boolean; editing?: Billing }>({
    open: false,
  });
  ...
  const cMut = useMutation({
    mutationFn: (d: BForm) =>
      createBilling({
        name: d.name,
        inputCostPer1KTokens: +d.input,
        outputCostPer1KTokens: +d.output,
        currency: d.currency,
      }),
    onSuccess: () => {
  ...
  const uMut = useMutation({
    mutationFn: ({ id, d }: { id: string; d: BForm }) =>
      updateBilling(id, {
        name: d.name,
        inputCostPer1KTokens: +d.input,
        outputCostPer1KTokens: +d.output,
  ...
  return (
    <div className="p-6 space-y-5 animate-fade-in">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-lg font-semibold">Model Pricing</h1>
          <p className="text-sm text-text-muted">Token-based pricing configs</p>
        </div>
        <button
              ...
              {billings.map((b: Billing) => (
                <tr
                  ...
                  <td className="py-3 px-4 text-text-secondary font-mono">
                    {b.inputCostPer1KTokens}
                  </td>
                  <td className="py-3 px-4 text-text-secondary font-mono">
                    {b.outputCostPer1KTokens}
                  </td>
                  <td className="py-3 px-4">
                    <span className="px-1.5 py-0.5 rounded bg-accent-amber/10 text-accent-amber text-[10px] font-medium">
                      {b.currency}
                    </span>
                  </td>
