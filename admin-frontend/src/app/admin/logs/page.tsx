      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-text-muted">
            {pagination
              ? `${pagination.total_items} total requests`
        ...
        <div className="flex items-center gap-2">
          ...
          <select
            ...
            className="px-2 py-1 rounded-lg border border-border-primary bg-bg-secondary text-xs text-text-primary focus:outline-none focus:border-border-active">
      ...
      {isLoading ? (
        <div className="space-y-3">
          {[...Array(6)].map((_, i) => (
            <div
              key={i}
              className="h-12 rounded-xl bg-bg-card border border-border-primary animate-pulse"
            />
          ))}
        </div>
      ) : !logs.length ? (
          ...
          <table className="w-full text-xs">
            ...
            <tbody>
              {logs.map((log) => {
                ...
                const provider =
                  typeof log.provider === "object" ? log.provider : null;
                return (
                  <tr
                    key={log._id}
                    ...
                    <td className="py-2.5 px-3">
                      <span className="px-1.5 py-0.5 rounded bg-accent-blue/10 text-accent-blue text-[10px]">
                        {provider?.name || "—"}
                      </span>
                    </td>
                    <td className="py-2.5 px-3 text-right text-text-secondary font-mono">
                      {log.promptTokens.toLocaleString()}
                    </td>
                    <td className="py-2.5 px-3 text-right text-text-secondary font-mono">
                      {log.completionTokens.toLocaleString()}
                    </td>
                    <td className="py-2.5 px-3 text-right text-text-primary font-mono font-medium">
                      {log.totalTokens.toLocaleString()}
                    </td>
                    <td className="py-2.5 px-3 text-right text-accent-amber font-mono">
                      ₹{log.cost.toFixed(4)}
                    </td>
                    <td className="py-2.5 px-3 text-right text-text-secondary font-mono">
                      {log.latencyMs}ms
                    ...
                    <td className="py-2.5 px-3 text-center">
                      <span
                        className={`px-1.5 py-0.5 rounded text-[10px] font-medium ${log.status === "success" ? "bg-accent-emerald/10 text-accent-emerald" : "bg-accent-rose/10 text-accent-rose"}`}>
                        {log.status}
                      </span>
                    </td>
